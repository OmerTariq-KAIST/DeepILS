"""
A simple code has been added to interpolate the camera pose data and synchronize the data with desired sampling rate
"""
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', type=str, default='', help='Path to a dataset folder.')
    parser.add_argument('--skip_front', type=int, default=120, help='Number of discarded records at beginning.')
    parser.add_argument('--skip_end', type=int, default=120, help='Numbef of discarded records at end')
    ...
    args.list = current_dir + '/list_data.txt'
    dataset_list = []
    root_dir = ''
    if args.path:
    ...
    time_factor = int(new_sampling / 30)
    ...
    for dataset in dataset_list:
        ...
        if os.path.exists(data_root + '/processed' + '/data.csv') and not args.recompute:
            ...
            position[:, 0] = pose_data[:, 1]
            position[:, 1] = -pose_data[:, 3]
            position[:, 2] = pose_data[:, 2]
            ...
            assert 28 < output_samplerate < 32, 'Wrong output sample rate: %f' % output_samplerate
            ...
            new_pose_data = interpolate_quaternion_linear(pose_data[:, -4:], output_timestamp, new_output_timestamp)
            new_pose_data[0, :] = pose_data[0, -4:]
            ...
            # plt.plot(output_position[:, 0], output_position[:, 1] , '-')
            # plt.title('Track 1')
            # plt.axis('equal')
            # plt.legend(['Position','New_Position'])
            # plt.show()
            # plt.legend(['ATE:{:.3f}'.format(ate)])

            print('Pose sample rate: {:2f}Hz'.format(new_output_samplerate))
            ...
            output_gyro_linear = interpolate_3dvector_linear(gyro_data[:, 1:], gyro_data[:, 0], new_output_timestamp)
            output_accelerometer_linear = interpolate_3dvector_linear(acce_data[:, 1:], acce_data[:, 0],
                                                                      new_output_timestamp)
            ...
            # position = np.zeros([len(output_timestamp) , 3])
            # position[:,0] = new_x
            # position[:,1] =  new_y
            # position[:,2] =  new_z
            # zero_pad_length = len(output_timestamp)-len(pose_data)
            # zero_pad = np.zeros([len(pose_data) , 4])
            # new_pos = np.concatenate([pose_data[:, -4:] , zero_pad])
            data_mat = np.concatenate([new_output_timestamp[:, None], output_gyro_linear,
