...
class ChannelAttention(nn.Module):
    def __init__(self, in_planes, ratio=16):
        ...
        self.avg_pool = nn.AdaptiveAvgPool1d(1) # both changed from 2D
...
class BasicBlock1D(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, kernel_size, stride=1, dilation=1, downsample=None):
        ...
        self.conv1 = conv_dw(in_planes, out_planes, kernel_size, stride, dilation)
        #self.bn1 = nn.BatchNorm2d(planes)
        self.bn1 = nn.BatchNorm1d(out_planes)
        self.relu = nn.ReLU(inplace=True)
        #Ablation experiment
        #self.relu = nn.ELU()
        self.conv2 = conv_dw(out_planes, out_planes, kernel_size)
...
""" adding the modules from deepils """
