...
class Bottleneck1D(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, kernel_size, stride=1, dilation=1, downsample=None):
        ...
        self.conv2 = conv3(in_planes, out_planes, kernel_size, stride, dilation)
        self.bn2 = nn.BatchNorm1d(out_planes)
...
class ResNet1D(nn.Module):
    ...
    def reset_output_block(self, block_type, num_outputs, **kwargs):
        """
        Reset the block of the final output layer in the ResNet1D model.

        Args:
            new_output_block (nn.Module): The new output block to replace the existing one.
        """
